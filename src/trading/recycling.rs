...
async fn recycle(database: &Pool<Sqlite>, user: UserId, emojis: &EmojisWithCounts) -> Emoji {
	...
	let (trade_offer, random_emoji) = loop {
		let random_emoji = Emoji::random();
		if let Ok(trade_offer) = TradeOffer::new(
			user,
			UserId(0),
			emojis.clone(),
			EmojisWithCounts::from_iter([(random_emoji, 1)]),
		) {
			break (trade_offer, random_emoji);
		}
	};
	...
	for (emoji, count) in emojis.into_iter() {
...
pub async fn execute(
	...
	let emoji = recycle(database, interaction.user.id, &emojis).await;
